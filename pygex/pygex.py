...
class pygex:
    ...
    def post2nfa(self, postfix):
        ...
        for char in postfix:
            ...
            if char == '.':
                ...
                frag_stack.append(frag(e1.start_state, e2.out_states))
            elif char == '|':
                e2 = frag_stack.pop()
                e1 = frag_stack.pop()
                s = state(split, e1.start_state, e2.start_state)
                frag_stack.append(frag(s, self._append(e1.out_states, e2.out_states)))
            elif char == '?':
                e = frag_stack.pop()
                s = state(split, e.start_state, None)
                frag_stack.append(frag(s, self._append(e.out_states, self._list1(s.set_out1))))
            elif char == '*':
                e = frag_stack.pop()
                s = state(split, e.start_state, None)
                self._patch(e.out_states, s)
                frag_stack.append(frag(s, self._list1(s.set_out1)))
            elif char == '+':
                e = frag_stack.pop()
                s = state(split, e.start_state, None)
                self._patch(e.out_states, s)
                frag_stack.append(frag(e.start_state, self._list1(s.set_out1)))
            else:
                s = state(char, None, None)
                frag_stack.append(frag(s, self._list1(s.set_out)))
        e = frag_stack.pop()
    ...
    def _print(self, s1, s2, depth):
        ...
        if s2 == None:
            ...
            print(('%s %s %s') % (str(depth), s1.char, s2.char))
        elif s1 != None:
            print(('%s %s %s') % (str(depth), s1.char, "None"))
            return
        else:
            print(('%s %s %s') % (str(depth), "None", s2.char))
